...
Pitty CLI — 一个基于 DeepSeek API 的 AI 编程助手 CLI。纯 Node.js 实现，零外部依赖。
...
node pitty.js --init          # 创建默认配置文件 ~/.pitty.json
```

## 架构

**入口:** `pitty.js` — CLI 参数解析，三种模式（REPL / 单次 / 管道）
...
- `config.js` — 配置管理。读写 `~/.pitty.json`，默认值：模型 `deepseek-chat`、baseUrl `https://api.deepseek.com`、maxTokens 4096、temperature 0
- `logger.js` — 日志。写入 `/tmp/pitty/<startup_timestamp>.log`，记录请求/响应/错误

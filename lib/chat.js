...
async function run(input, messagesOrOpts = [], opts = {}) {
  ...
  while (turnCount < maxTurns) {
    ...
    const tokenState = { realIn, realOut, dispIn, dispOut };
    ...
    if (!hasToolCalls) {
      if (ctx.lastUsage) {
        const p = ctx.lastUsage.prompt_tokens ?? ctx.lastUsage.input_tokens ?? 0;
        const c = ctx.lastUsage.completion_tokens ?? ctx.lastUsage.output_tokens ?? 0;
        if (p > 0 || c > 0) {
          const ctxTokens = p + c;
          sb.barFinalize(_('chat.ctx') + ':' + formatNum(ctxTokens) + '/' + formatNum(maxContext) +
            ' ↑' + formatNum(tokenState ? tokenState.dispIn : 0) +
            ' ↓' + formatNum(tokenState ? tokenState.dispOut : 0));
        } else {
          sb.barFinalize('');
        }

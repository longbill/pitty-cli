...
async function resolveToolApprovals(toolCalls, messages, confirm, sb, ac) {
    ...
    for (const item of classified) {
      ...
      if (item.needAudit && auditResult) {
        ...
        if (review && !review.safe) {
          sb.pause();
          const confirmation = confirm ? await confirm(`${item.tc.function.name}: ${review.reason}`, ac.signal) : { ok: false, userInput: '' };
          sb.resume();
          if (!confirmation.ok) {
            denied.push({
              id: item.tc.id,
              name: item.tc.function.name,
              result: { error: _('chat.denied'), ...(confirmation.userInput ? { userInput: confirmation.userInput } : {}) },
            });
            continue;
          }
        }
  ...
  for (const item of classified) {
    ...
    if (needsConfirm) {
      let args = {};
      try { args = JSON.parse(item.tc.function.arguments); } catch {}
      const desc = formatToolConfirmation(item.tc.function.name, args);
      sb.pause();
      const confirmation = confirm ? await confirm(desc, ac.signal) : { ok: false, userInput: '' };
      sb.resume();
      if (!confirmation.ok) {
        denied.push({
          id: item.tc.id, name: item.tc.function.name,

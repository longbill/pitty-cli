...
function startThinking() {
  ...
  let interval = setInterval(() => {
    ...
    if (reasoning) {
      ...
      process.stdout.write(CLEAR_LINE + `思考中 ${elapsed}s  ${snippet}`);
    } else {
      process.stdout.write(CLEAR_LINE + `思考中 ${elapsed}s`);
    }
  }, 200);
...
async function run(input, opts = {}) {
  ...
  while (turnCount < maxTurns) {
    ...
    // If no content was output, clear thinking on request complete
    if (!contentStarted) {
      thinking.done();
    }

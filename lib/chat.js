...
function flushTable() {
  ...
  const rows = mdTableBuffer.map(parseTableRow);
  ...
  for (const row of rows) {
    for (let i = 0; i < row.length; i++) {
      ...
      // Rough CJK width: CJK chars count as 2
      let w = 0;
  ...
  for (let r = 0; r < rows.length; r++) {
    const isHeader = r === 0;
    const isSep = r === 1 && rows[r].every(isTableSeparator);
...
function renderSingleLine(line) {
  ...
  // Inline formatting (bold before italic to avoid conflict)
  line = line.replace(/\*\*(.+?)\*\*/g, BOLD + '$1' + RESET);
  line = line.replace(/\*(.+?)\*/g, ITALIC + '$1' + RESET);
  line = line.replace(/`(.+?)`/g, GREEN + '$1' + RESET);

...
async function run(input, opts = {}) {
  ...
  while (turnCount < maxTurns) {
      ...
      response = await chat(
        ...
        (delta) => {
          ...
          for (const line of lines) {
            barWriteLine(renderLine(line));
          }
        },
        (chunk) => {
          reasoningChars += chunk.length;
          estOutputTokens = totalOutputTokens + Math.round((reasoningChars + totalOutputChars) / 2.5);
          barUpdate('思考中... [~' + formatNum(estOutputTokens) + ' tokens]');
        },
        (usage) => {
          lastUsage = usage;
    ...
    if (lineBuffer) {
      barWriteLine(renderLine(lineBuffer));
    }
    ...
    for (const tc of response.toolCalls) {
      ...
      process.stderr.write(`${emoji} ${tc.function.name}(${args})\n`);
    }

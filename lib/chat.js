...
function buildUsageHandler(ctx, tokenState) {
  return (usage) => {
    ...
    // DEBUG: print raw usage and parsed result
    ctx.sb.barWriteLine('\x1b[33m[DEBUG raw usage] ' + JSON.stringify(usage) + '\x1b[0m');
    ctx.sb.barWriteLine('\x1b[33m[DEBUG parsed]     ' + JSON.stringify(parsed) + '\x1b[0m');
...
function buildContentHandler(ctx, maxContext, tokenState) {
  return (chunk) => {
    ...
    if (tokenState) {
      tokenState.dispOut = tokenState.realOut + ctx.estTokens;
      ctx.sb.setTokenInfo(tokenState.dispIn, tokenState.dispOut);
    }
  };
}
  ...
  while (turnCount < maxTurns) {
    ...
    if (!hasToolCalls) {
      ...
      if (tokenState && (tokenState.realIn > 0 || tokenState.realOut > 0)) {
        sb.barFinalize(
          _('chat.ctx') + ':' + formatNum(currentContextSize) + '/' + formatNum(maxContext) +
          ' ' + _('chat.round') + ' ↑' + formatNum(tokenState.realIn) +
          ' ↓' + formatNum(tokenState.realOut) +
          gray(' ' + _('chat.session') + ' ↑' + formatNum(sessionRealIn) + ' ↓' + formatNum(sessionRealOut))
        );
      } else {
        sb.barFinalize('');
      }

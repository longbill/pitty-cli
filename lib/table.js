...
function wrapText(text, width) {
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      // Collect full ANSI escape sequence
      if (ch === '\x1b' && line[i + 1] === '[') {
        let end = i + 2;
        while (end < line.length && line[end] !== 'm') end++;
        if (end < line.length) end++; // include 'm'
      ...
      const chW = isCJK(ch) ? 2 : 1;

      if (curW + chW > width) {
        // Try to break at a word boundary for ASCII
        if (!isCJK(ch) && ch !== ' ' && cur.length > 0) {
          const lastSpace = cur.lastIndexOf(' ');
          if (lastSpace > 0) {
            result.push(cur.slice(0, lastSpace));
            cur = activeCodes + cur.slice(lastSpace + 1);
            curW = strlen(cur);
          } else {
            result.push(cur);
            cur = activeCodes;
...
class Table {
  constructor(opts = {}) {
    ...
    this._terminalWidth = opts.terminalWidth || ((process.stdout.columns || 81) - 1);

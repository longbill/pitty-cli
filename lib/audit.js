...
// Consecutive risky-call counter for audit-pass mode
let riskyStreak = 0;

function resetRiskyStreak() {
  riskyStreak = 0;
}
...
async function auditToolCalls(toolCalls, messages) {
  ...
  if (!auditModelRef) {
    // No audit model configured — treat all as safe
  ...
  // Build a concise message describing what to review
  ...
  try {
    await chat(
      ...
      [], // no tools for audit agent
      (delta) => { fullContent += delta; },
      () => {}, // no reasoning callback needed
      () => {}, // no usage callback needed
      undefined, // no signal
      providerConfig,
    );
  } catch {
...
function shouldAskUser(mode, allSafe) {
  if (mode === 'audit-ask') return !allSafe;
  if (mode === 'audit-pass') {
    if (allSafe) {
      riskyStreak = 0;
      return false;
    ...
    riskyStreak++;
    return riskyStreak >= 3;
  }
  return false;
}

module.exports = { auditToolCalls, shouldAskUser, resetRiskyStreak };

...
function appendTaskOutput(task, stream, chunk) {
  ...
  task[key] += chunk;
  if (task[key].length > MAX_STREAM_CHARS) {
    const removed = task[key].length - MAX_STREAM_CHARS;
    task[key] = task[key].slice(-MAX_STREAM_CHARS);
    task[truncatedKey] = true;
    if (key === 'stdout') task.stdoutOffset = Math.max(0, task.stdoutOffset - removed);
    else task.stderrOffset = Math.max(0, task.stderrOffset - removed);
  }
  notifyTaskOutput(task);
}
...
function registerBackgroundChild({ command, cwd, child, stdout = '', stderr = '', startTime = new Date(), captureOutput = true }) {
  ...
  if (captureOutput) {
    child.stdout.on('data', (d) => { appendTaskOutput(task, 'stdout', d.toString()); });
    child.stderr.on('data', (d) => { appendTaskOutput(task, 'stderr', d.toString()); });
  }
  child.on('close', (code) => {
  ...
  child.on('error', (err) => {
    ...
    task.exitCode = -1;
    task.endTime = new Date();
    appendTaskOutput(task, 'stderr', err.message);
    const outputTimer = outputNotifyTimers.get(task);
    if (outputTimer) {
      clearTimeout(outputTimer);
      outputNotifyTimers.delete(task);
...
function stopTask(id) {
  ...
  if (task.status === 'running') {
    ...
    forceKillProcessTree(task.child);
    task.status = 'stopped';
    task.endTime = new Date();
  }
  return true;
...
function resetForTests() {
  for (const task of tasks.values()) {
    if (task.status === 'running') killProcessTree(task.child, 'SIGTERM');
  }
  tasks.clear();
  nextTaskNumber = 1;
}
...
function readTaskOutput(id, opts = {}) {
  ...
  let truncated = task.stdoutTruncated || task.stderrTruncated;

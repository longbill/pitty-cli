...
function startRepl() {
  ...
  function extractBangCommands(text) {
    ...
    // Match inline !commands — backtick-wrapped (captures full command with args) or bare !word
    const inlineRegex = /(?:^|[\s;(])`(!(?:[^`]|\\`)+)`|(?:^|[\s;(])(![\w\-./@"'=+]+)/gm;
    while ((match = inlineRegex.exec(text)) !== null) {
      // group 1 = backtick-wrapped (!cmd args), group 2 = bare !word

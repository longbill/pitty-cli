...
const { createConfirmFn } = require('./confirm.js');
const { formatBackgroundTaskReminder, formatShellReminder } = require('./replReminders.js');
const { run } = chat;

function makeConfirmFn() {
  return createConfirmFn({ showSuccess: true });
}
...
function startRepl() {
  ...
  function buildShellReminder() {
    return formatShellReminder(recentShellOutputs);
  }
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      function handleEnter() {
        ...
        if (cursorPos > 0 && inputBuffer[cursorPos - 1] === '\\') {
          const curRow = getVisualPos(line.length).row;
          const savedPos = cursorPos;
          process.stdout.write('\r\n');
          inputBuffer = line.slice(0, savedPos - 1) + '\n' + line.slice(savedPos);
          cursorPos = savedPos;
          process.stdout.write('\x1b[' + (curRow + 1) + 'A\r\x1b[J');
          process.stdout.write(promptStr + inputBuffer);
          process.stdout.write('\x1b[J');
          cursorPos = inputBuffer.length;
          moveCursorTo(savedPos);
          cursorPos = savedPos;
          return true;
        }
        return false;

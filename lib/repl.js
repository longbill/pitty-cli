...
function startRepl() {
  ...
  if (mode !== 'web-only') {
    ...
    ensureRg().then(available => {
      if (!available) {
        const rgPrompt = [
          '当前系统未安装 ripgrep (rg)。rg 是一个高性能的代码搜索工具，比传统 grep 快数倍。',
          '',
          '请向用户说明以下信息：',
          '1. rg 能大幅提升文件搜索和内容搜索的速度',
          '2. 安装后 Grep 和 Glob 工具将使用 rg 作为后端，搜索效率会显著提高',
          '3. 在 Debian/Ubuntu 上可执行: sudo apt install ripgrep',
          '4. 在 macOS 上可执行: brew install ripgrep',
          '5. 其他平台安装方式: https://github.com/BurntSushi/ripgrep#installation',
          '',
          '请询问用户是否要安装 rg，如果用户同意，请使用 Bash 工具执行对应的安装命令。',
        ].join('\n');
        runModelTurn(rgPrompt).catch(() => {});
      }
    }).catch(() => {});
  }

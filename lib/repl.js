...
function startRepl() {
  ...
  if (mode !== 'web-only') {
    ...
    ensureRg().then(available => {
      if (!available) {
        const rgPrompt = '当前系统未安装 ripgrep (rg)，这会导致 Pitty 搜索文件和代码的时候变慢。请给用户讲一下 ripgrep 是什么，有什么优点。结合用户当前操作系统，讲一下安装方式。如果你有 Bash 工具可以用，询问用户是否需要你帮用户安装。如果你没有 Bash 工具，就提示用户在输入框里面输入 !command 来手动执行安装命令（这里的 command 换成对应的安装命令）。';

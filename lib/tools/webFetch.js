...
async function fetchWithPolicy(rawUrl, opts) {
  // AbortSignal.timeout() 是 Node 17.3+ 特性，手动兼容 Node 16
  const timeoutAc = new AbortController();
  const timer = setTimeout(() => timeoutAc.abort(), FETCH_TIMEOUT);

  // 如果外部有 signal，任一 abort 都取消
  const extSignal = opts.signal;
  const onAbort = () => { clearTimeout(timer); timeoutAc.abort(); };
  if (extSignal) {
    if (extSignal.aborted) { clearTimeout(timer); timeoutAc.abort(); }

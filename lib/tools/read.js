...
const tool = {
  ...
  async execute(args, opts) {
    ...
    try {
      if (!opts?.bypassPathCheck) assertAllowedPath(filePath);
      ...
      if (args.offset !== undefined && args.limit !== undefined) {
        ...
        const numbered = lines.map((line, i) => `${args.offset + i + 1}\t${line}`);
      ...
      return { content: numbered.join('\n'), size: stat.size, path: filePath, _mtime: stat.mtimeMs };
    } catch (err) {

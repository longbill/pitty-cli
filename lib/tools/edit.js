...
const tool = {
  name: 'Edit',
  description: 'Perform a targeted edit on a file by replacing an exact string match. Use this to make surgical changes without rewriting the whole file. When editing text from Read tool output, ensure you preserve the exact indentation (tabs/spaces) as it appears AFTER the line number prefix. The line number prefix format is: line number + tab. Everything after the tab is the actual file content to match. Never include any part of the line number prefix in the old_string or new_string.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  execute(args, opts) {
    ...
    try {
      ...
      if (lastMtime == null) {
        return { error: `文件 ${filePath} 尚未被读取，请先用 Read 工具读取后再编辑` };
      }
      const mtimeStat = fs.statSync(filePath);
      if (mtimeStat.mtimeMs !== lastMtime) {

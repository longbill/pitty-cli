const os = require('os');
const fs = require('fs');
const { buildDscMdPrompt } = require('./memory.js');
...
function buildSystemPrompt(tools) {
  ...
  const dscMdContent = buildDscMdPrompt(cwd);

  const sections = [
    getIdentitySection(),
    ...(dscMdContent ? [dscMdContent] : []),
    getSystemSection(),
...
function getIdentitySection() {
  return `你是 DSC（DeepSeek CLI），一个交互式编程助手。请使用可用的工具帮助用户完成软件工程任务。
...
function getUsingToolsSection({ hasBash, hasRead, hasWrite, hasEdit, hasGlob, hasGrep }) {
  ...
  lines.push(`- 调用工具时，必须通过 API 的 tool_calls 机制发起真实的 function call，不要在 content 文本中输出 JSON、代码块或模拟工具调用的文字。`);

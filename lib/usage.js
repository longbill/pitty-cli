...
// 一旦检测到某 provider 的 prompt_tokens 已扣除缓存，后续调用不再重复减
let _promptExcludesCache = false;
...
function parseUsage(data) {
  ...
  if (typeof usage.input_tokens === 'number') {
    ...
    if (prompt < cached_input_tokens) {
      // prompt_tokens 已经是扣除缓存后的值，记住这一行为
      _promptExcludesCache = true;
      input_tokens = prompt;
    } else if (_promptExcludesCache) {
      // 该 provider 已验证 prompt_tokens 不含缓存
      input_tokens = prompt;
    } else {
      // prompt_tokens 包含缓存，需扣除

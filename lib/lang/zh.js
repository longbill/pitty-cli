module.exports = {
  ...
  ReadGroup: (files) => `让我看看这几个文件: ${files.join(' ')}`,
  Write:     (args) => `写入文件 ${args.file_path || ''}`,
  Edit:      (args) => `编辑文件 ${args.file_path || ''}`,
  Bash:      (args) => `执行命令: ${(args.command || '').slice(0, 80)}`,
...
用法:
  ...
  pitty --init               创建默认配置文件 ~/.pitty.json
...
配置文件: {path}
  ...
  - tools:     工具开关配置`,
    ...
    toolsLabel:   '可用工具',
    promptLabel:  'pitty',
    exitHint:     '(再按一次 Ctrl+C 退出)',
    clearDone:    '(会话已清除)',
    bye:          '再见！',
    canceled:     '(已取消)',
    errorPrefix:  '错误: ',
    cmdHelp:      '命令:  /clear /c 清除会话  |  /exit /q 退出  |  /help /h 帮助',
    confirmPrefix: '允许执行?',
  },

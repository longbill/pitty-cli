module.exports = {
  ...
  Read:      (args) => {
               ...
               return `Reading file: ${rel}`;
             },
  ReadGroup: (files) => {
               const cwd = process.cwd();
               const rels = files.map(f => f.startsWith(cwd + '/') ? f.slice(cwd.length + 1) : f);
               return `Reading batch: ${rels.join(' ')}`;
             },
  Write:     (args) => `Writing to ${args.file_path || ''}`,
  Edit:      (args) => `Editing ${args.file_path || ''}`,
  Bash:      (args) => `Running: ${args.command || ''}`,
  Glob:      (args) => `Searching files: ${args.pattern || ''}${args.path ? ` (${args.path})` : ''}`,
  Grep:      (args) => `Searching content: ${args.pattern || ''}${args.path ? ` (${args.path})` : ''}`,
  ...
  toolErrors: {
    ...
    contentExceeds: ({ size, limit }) => `File content (${size}) exceeds maximum allowed size (${limit}). Use offset and limit parameters to read specific portions of the file, or search for specific content instead of reading the whole file.`,
    linesExceed:   ({ lines, max }) => `Cannot read ${lines} lines, maximum is ${max}`,
  },
...
Usage:
  ...
  pitty --init               Create default ~/.pitty.ini
  pitty --switch-model       Select and switch current model
...
Config: {path}
  ...
  - tools:     Object mapping tool names to true/false`,
    ...
    modeLabel:    'Mode',
    modeDesc: {
      'web-only':   'WebFetch only, no filesystem access',
      'read-only':  'Read files, search content, fetch URLs',
      'ask':        'Confirmation required for write/exec commands',
      'audit':      'Audit agent reviews operations, risky ones need approval',
      'accept-all': 'All tools auto-approved (write/exec have countdown)',
    },
  ...
  chat: {
    ...
    running:      'Running...',
    commandRunning: 'Command running... (Ctrl+B to run in background)',
    tokens:       'tokens',
    ctx:          'ctx',
    up:           '↑',
    down:         '↓',
    ...
    autoAcceptCountdown: (v) => `Auto-executing in ${v.seconds}s... (Ctrl+C to cancel)`,
  },
...
- Don't add a colon before tool calls. Say "Let me look at the file." instead of "Let me look at the file:".`,
    ...
    usingTools: {
      ...
      background:   `For long-running Bash commands (over 10 seconds), consider using BackgroundCreate to run them in the background so you can continue with other work`,
      noBash:       `If Bash tool is not available but a shell command is needed, ask the user to type !command in the input box (e.g. !npm test)`,
